...
save_module = config.tvm.save_module
...
class _conv:
    ...
    def _call(
        ...
        if self.exist_module(module) == False:
            ...
            if config.tvm.tuning:
                try:
                    lib = _conv.tune_with_ms(mod, target, {})
                except Exception:
                    lib = _conv.tune_with_relay(mod, target, {})
            else:
                lib = _conv.tune_with_relay(mod, target, {})
            if save_module:
        ...
        m.set_input_zero_copy("data", tvm.nd.from_dlpack(x.detach().contiguous()))
        m.set_input_zero_copy("weight", tvm.nd.from_dlpack(w.detach().contiguous()))
        m.run()
        res = m.get_output(0)
    ...
    @staticmethod
    def tune_with_ms(mod, target, params):
        import tempfile
